...
const createContentController = async (req, res) => {
  try {
    const { course, chapter, contentvideo, contentfile } = req.body;
    //Course
    ...
    if (instructorExist) {
      ...
      const contentVideoInfo = await ContentVideo.findById({
        _id: contentvideo,
      });
    ...
    if (instructorExist) {
      ...
      if (getAllContent) {
        for (const field of getAllContent) {
          ...
          const chapter = await Chapter.findById({
            _id: field.chapterDetailes,
          });
          if (course) {
            field.chapterDetailes = chapter;
          }
          const contentVideo = await ContentVideo.findById({
            _id: field.contentVideoDetailes,
          });
          if (course) {
            field.contentVideoDetailes = contentVideo;
          }
          const contentFile = await ContentFile.findById({
            _id: field.contentFileDetailes,
          });
          if (course) {
      ...
      if (keyword !== "undefined") {
        ...
        const searchCheck = (field) => {
          if (
            field.courseDetailes.name.match(finalQueryVal) ||
            field.chapterDetailes.chapterName.match(finalQueryVal) ||
            field.contentFileDetailes.name.match(finalQueryVal) ||
            field.contentVideoDetailes.thumbnail.match(finalQueryVal)
          ) {
            return field;
          }
        };
        const getSearchData = getAllContent.filter(searchCheck);
        // .skip(skip)
        // .limit(bookPerPage);
      ...
      if (ObjectId.isValid(id)) {
        ...
        if (getSingleContent) {
          ...
          const courseInfo = await Course.findById({
            _id: getSingleContent.courseDetailes,
          });
          ...
          const chapterInfo = await Chapter.findById({
            _id: getSingleContent.chapterDetailes,
          });
          ...
          // }
          const contentVideoInfo = await ContentVideo.findById({
            _id: getSingleContent.contentVideoDetailes,
          });
          getSingleContent.contentVideoDetailes = contentVideoInfo;
          const contentFileInfo = await ContentFile.findById({
            _id: getSingleContent.contentFileDetailes,
          });

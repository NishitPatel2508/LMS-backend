...
const createContentVideoController = async (req, res) => {
  ...
  try {
    ...
    if (instructorExist) {
      const videoLinkExist = await ContentVideo.findOne({
        videoLink: videoLink,
      });
      if (videoLinkExist) {
    ...
    if (instructorExist) {
      ...
      if (keyword != "undefined") {
        const searchCheck = (field) => {
          if (
            field.thumbnail.match(finalQueryVal) ||
            field.chapter.chapterName.match(finalQueryVal)
          ) {
            return field;
          }
        };
        const getSearchData = getAllContentVideo.filter(searchCheck);
    ...
    if (instructorExist) {
      if (ObjectId.isValid(id)) {
        ...
        if (contentVideoExist) {
          const contentVideoUpdate = await ContentVideo.findByIdAndUpdate(
            id,
            req.body,
            {
              new: true,
  ...
  try {
    ...
    if (instructorExist) {
      if (ObjectId.isValid(id)) {
        const contentVideoExist = await ContentVideo.findOne({ _id: id });
        if (contentVideoExist) {
          const contentVideoDelete = await ContentVideo.findByIdAndDelete({
            _id: id,
          });
          return res.status(HTTPStatusCode.OK).json({

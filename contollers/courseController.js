...
const createCourseController = async (req, res) => {
  ...
  try {
    ...
    const programmingLanguage = await ProgrammingLanguage.findById({
      _id: programmingLanguageId,
    });
    const courseExist = await Course.find({ name: name })
      .where("createdBy")
      .equals(userid);
    ...
    if (instructorExist) {
      ...
      for (const fieldNames of getAllCourse) {
        ...
        const subCategory = await Subcategory.findById({
          _id: fieldNames.subCategory,
        });
        if (subCategory) {
          fieldNames.subCategory = subCategory;
        }
        const programmingLanguage = await ProgrammingLanguage.findById({
          _id: fieldNames.programmingLanguage,
        });
      ...
      if (keyword !== "undefined") {
        // const getSearchData = await Course.find({
        //   $or: [
        //     { name: finalQueryVal },
        //     { "category.categoryName": finalQueryVal },
        //     { "subCategory.subCategoryName": finalQueryVal },
        //     { "language.languageName": finalQueryVal },
        //     { "programmingLanguage.programmingLanguageName": finalQueryVal },
        //     { level: finalQueryVal },
        //   ],
        // })
        //   .skip(skip)
        ...
        const searchCheck = (field) => {
          if (
            ...
            field.subCategory.subCategoryName.match(finalQueryVal) ||
            field.programmingLanguage.programmingLanguageName.match(
              finalQueryVal
            ) ||
            field.level.match(finalQueryVal)
          ) {
            return field;
          }
        };
        const getSearchData = getAllCourse.filter(searchCheck);
        // .skip(skip)
        // .limit(bookPerPage);
      ...
      if (ObjectId.isValid(id)) {
        ...
        if (getSingleCourse) {
          ...
          const chapter = await Chapter.find({
            createdBy: getSingleCourse.createdBy,
          });
          if (chapter) {
            getSingleCourse.chapter = chapter;
          }
          const contnet = await Content.find({
            courseDetailes: getSingleCourse._id,
          });
          const allContent = [];
          if (contnet) {
            ...
            for (const filed of allContent) {
              for (const i of filed) {
                const chapter = await Chapter.findOne({
                  _id: i.chapterDetailes,
                });
                if (chapter) {
                  i.chapterDetailes = chapter;
                }
                const contentvideo = await ContentVideoes.findOne({
                  _id: i.contentVideoDetailes,
                });
                if (contentvideo) {
                  i.contentVideoDetailes = contentvideo;
                }
                const contentfile = await ContentFile.findOne({
                  _id: i.contentFileDetailes,
                });
                if (contentfile) {
...
const getAllCourseForUserController = async (req, res) => {
  ...
  // const finalQueryVal = new RegExp(keyword,'i')
  try {
    // queryForPrice.price = {$gte:keyword}
    const getAllCourse = await Course.find()
      .skip(pages * bookPerPage)
      .limit(bookPerPage);
    for (const fieldNames of getAllCourse) {
      const category = await Category.findById({ _id: fieldNames.category });
      if (category) {

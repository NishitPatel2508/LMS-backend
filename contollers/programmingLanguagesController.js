...
const createProgrammingLanguageController = async (req, res) => {
  ...
  try {
    ...
    if (instructorExist) {
      ...
      const programmingLanguageExistCheck = await ProgrammingLanguage.findOne({
    ...
    if (instructorExist) {
      ...
      if (keyword != "undefined") {
        const searchCheck = (field) => {
          if (
            field.programmingLanguageName.match(finalQueryVal) ||
            field.subCategory.subCategoryName.match(finalQueryVal)
          ) {
            return field;
          }
        };
        const getSearchData = getAllProgrammingLanguage.filter(searchCheck);
  ...
  try {
    ...
    if (instructorExist) {
      if (ObjectId.isValid(id)) {
        const singleProgrammingLanguage = await ProgrammingLanguage.findOne({
          _id: id,
        });
        const subCategoryInfo = await Subcategory.findById({
          _id: singleProgrammingLanguage.subCategory,
        });
        if (subCategoryInfo) {
...
module.exports = {
  ...
  updateProgrammingLanguageController,
  deleteProgrammingLanguageController,
};
